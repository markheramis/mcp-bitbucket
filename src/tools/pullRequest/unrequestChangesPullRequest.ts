...
/**
 * Register the pull request change-request removal tool.
 * @param server The MCP server instance that exposes the tool.
 * @returns Nothing.
 */
export function registerUnrequestChangesPullRequestTool(server: McpServer): void {
  server.tool(
    ...
    async ({ workspace, repository, prId }: { workspace: string; repository: string; prId: number }) => {
      try {
        const response = await client.pullrequests.deleteChangeRequest({
          workspace,
